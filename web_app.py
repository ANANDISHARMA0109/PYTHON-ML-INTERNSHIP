...
yoga_data = pd.read_csv(r"Yoga_asanas - Sheet1.csv") #
benefit_poses = dict()
asana_desc = dict()
csvfile = open(r'Yoga_asanas - Sheet1.csv', newline='\n') #
...
for i in  benefits:
    ...
    for j in  i.strip().split():
        problem+=j
    benefit_lst.append(problem.lower())

tfidf = TfidfVectorizer()
benefit_vectors = tfidf.fit_transform(benefit_lst)
...
def recommend_yoga_asanas(user_input):
    ...
    if similarity_scores[0][recommended_index]<0.6:
        return "", ""
...
def yoga_recommender(user_problem, show_descriptions):
    input_lst=user_problem.strip().split()
    user_input=""
    for i in  input_lst:
        user_input+=i
    asanas, descriptions = recommend_yoga_asanas(user_input.lower())
    if asanas=="":
        asanas_str="No match found!"
    else:
